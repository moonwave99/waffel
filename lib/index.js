...
var Promise, Waffel, _, async, cheerio, colors, fs, glob, i18n, markdown, marked, moment, nunjucks, path, pushserve, util, yaml,
...
module.exports = Waffel = (function() {
  ...
  Waffel.prototype.helpers = {
    url: function(name, data, options) {
      ...
      if (this.options.uglyUrls) {
        relativeUrl = (this._url(page, data, options)) || 'index';
        url = [this.options.domain, relativeUrl].join('/');
        if (this.options.displayExt) {
          return "" + url + this.options.outputExt;
        } else {
          return url;
        }
      } else {
  ...
  Waffel.prototype.generate = function() {
    ...
    return fs.ensureDirAsync(this.options.destinationFolder).then((function(_this) {
      return function() {
        ...
        tasks = [];
        languages = _this.options.localiseDefault ? languages : _this.options.languages.filter(function(l) {
  ...
  Waffel.prototype._launchServer = function() {
    ...
    return pushserve(opts, function() {
      var address;
      address = "http://localhost:" + opts.port;
      return console.log("--> waffel server waiting for you at " + address.green);
    });
  };
